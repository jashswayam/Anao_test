...
class AIOrchestrator:
    def __init__(self, l1_config, l2_config, timeout: int = 60):
        ...
        proxy_auth_id = os.getenv("PROXY_AUTH_ID")
        ...
        if proxy_host and proxy_port:
            if proxy_auth_id and proxy_password:
                proxy_url = f"http://{proxy_auth_id}:{proxy_password}@{proxy_host}:{proxy_port}"
            else:
                proxy_url = f"http://{proxy_host}:{proxy_port}"
        
        # Initialize a custom HTTP client with proxy and verification if provided
        client_kwargs = {
        ...
        if proxy_cert_path and os.path.exists(proxy_cert_path):
            client_kwargs["verify"] = proxy_cert_path
